...
export default function MobileVerificationDialog({ open, onClose, mobile }) {
  ...
  async function handleSend() {
    if (!mobile) {
      setStatus({ text: 'Mobile not available', type: 'error' });
      return;
    }
    setSending(true);
    setStatus({ text: '', type: '' });
    try {
      const res = await userAPI.sendMobileOtp(mobile);
      setStatus({ text: res.message || `OTP sent to ${mobile}`, type: 'info' });
      setSent(true);
      startTimer(300);
    } catch (err) {
  ...
  async function handleVerify() {
    ...
    setStatus({ text: '', type: '' });
    try {
      const res = await userAPI.verifyMobileOtp({ mobile, otp });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl w-full max-w-md p-6 shadow-lg z-10">
        ...
        <h3 className="text-xl font-semibold mb-2">Verify Mobile Number</h3>
        <p className="text-sm text-neutral-600 mb-4">Confirm ownership of this mobile number to verify your account.</p>

        <div className="mb-4">
          <label className="label text-sm">Mobile</label>
          <input type="tel" value={mobile || ''} disabled className="input bg-neutral-100 cursor-not-allowed" />
        </div>
